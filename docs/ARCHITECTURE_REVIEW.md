# DeployHub — Full Project Analysis

## What DeployHub Is

DeployHub is an **Internal Developer Platform (IDP)** — a self-service system where a developer pushes a repo URL, and the platform automatically builds a container image, deploys it to Kubernetes, and routes traffic to it via a unique subdomain. Think of it as a mini Heroku/Vercel built on top of EKS.
...
`POST /projects` → inserts a row in `projects` table with `status=Provisioning`. The **Controller** picks this up on its next reconciliation tick (every 60s), renders the [tenant.yaml.j2](file:///c:/Project/DeployHub/controller/templates/tenant.yaml.j2) template, and creates the Kubernetes Namespace, ResourceQuota, LimitRange, NetworkPolicy, RBAC, and ECR pull secret. Once the namespace is verified as existing, the project moves to `Ready`.
...
The [Worker](file:///c:/Project/DeployHub/worker/main.py) runs an infinite `XREADGROUP` loop. It validates the message, checks for idempotency (skips if already Building/Deploying/Running), transitions the deployment to `Building`, and hands the message off to `builder_queue`.

### 4. Builder builds the image
The [Builder](file:///c:/Project/DeployHub/builder/main.py) (scaled by KEDA from 0→N based on pending messages) clones the repo, runs `pack build --publish` (Cloud Native Buildpacks), streams logs to S3, checks ECR image scan results, and writes the final state to PostgreSQL. On success → `Deploying`. On failure → `Failed`.

### 5. Controller reconciles the deployment
The [Controller](file:///c:/Project/DeployHub/controller/main.py) sees `state=Deploying`, renders [app-deployment.yaml.j2](file:///c:/Project/DeployHub/controller/templates/app-deployment.yaml.j2) (Deployment + Service + HTTPRoute + PDB + ExternalSecret), applies it to K8s, and watches for ready replicas. Once replicas are healthy → `Running`. If replicas crash → `Failed`.

### 6. Traffic is routed
Envoy Gateway terminates TLS on `*.deployhub.jeneeldumasia.codes` and routes based on hostname `{dep-id-8chars}.{project-name}.deployhub.jeneeldumasia.codes`.
...
|-----------|----------|---------|-------|
| [api/main.py](file:///c:/Project/DeployHub/api/main.py) | Python/FastAPI | HTTP API, Redis enqueue | 791 |
| [api/auth.py](file:///c:/Project/DeployHub/api/auth.py) | Python | Auth0 JWT validation | 132 |
| [api/database.py](file:///c:/Project/DeployHub/api/database.py) | Python | DB connection + retention | 70 |
| [api/audit.py](file:///c:/Project/DeployHub/api/audit.py) | Python | Append-only audit logging | 71 |
| [worker/main.py](file:///c:/Project/DeployHub/worker/main.py) | Python | Redis consumer, state machine | 122 |
| [worker/state_machine.py](file:///c:/Project/DeployHub/worker/state_machine.py) | Python | PostgreSQL state transitions | 81 |
| [builder/main.py](file:///c:/Project/DeployHub/builder/main.py) | Python | Image builder (pack) | 305 |
| [controller/main.py](file:///c:/Project/DeployHub/controller/main.py) | Python | K8s reconciliation loop | 284 |
| [ui/](file:///c:/Project/DeployHub/ui) | Next.js/TS | Dashboard + deploy forms | — |
...
**File:** [api/main.py:685](file:///c:/Project/DeployHub/api/main.py#L685)
...
**File:** [worker/state_machine.py:64-71](file:///c:/Project/DeployHub/worker/state_machine.py#L64-L71)
...
**File:** [api/database.py:15-16](file:///c:/Project/DeployHub/api/database.py#L15-L16)
...
**Files:** [api/main.py:126](file:///c:/Project/DeployHub/api/main.py#L126), [builder/main.py:123](file:///c:/Project/DeployHub/builder/main.py#L123)
...
**File:** [api/main.py:661-744](file:///c:/Project/DeployHub/api/main.py#L661-L744)
...
**File:** [api/main.py:358-370](file:///c:/Project/DeployHub/api/main.py#L358-L370)
...
**File:** [controller/main.py:89-105](file:///c:/Project/DeployHub/controller/main.py#L89-L105)
...
**File:** [api/main.py:390-435](file:///c:/Project/DeployHub/api/main.py#L390-L435)
...
**File:** [controller/models.py:6-9](file:///c:/Project/DeployHub/controller/models.py#L6-L9)
...
All tenant images go to `deployhub-builds:${deployment_id}`. This means:
...
- Image tag space is flat — a single `deployhub-builds` repo will accumulate thousands of tags.

---

#### 12. NetworkPolicy egress allows 0.0.0.0/0 minus RFC1918

**File:** [controller/templates/tenant.yaml.j2:71-78](file:///c:/Project/DeployHub/controller/templates/tenant.yaml.j2#L71-L78)
...
The API stores env vars at `deployhub/{project_name}/` ([api/main.py:574](file:///c:/Project/DeployHub/api/main.py#L574)), but the controller's app-deployment template fetches from `deployhub/{project_name}/{deployment_name}` ([app-deployment.yaml.j2:19](file:///c:/Project/DeployHub/controller/templates/app-deployment.yaml.j2#L19)). These paths will never match, so environment variables set via the API will never appear in the deployed containers.

> [!CAUTION]
> The env vars feature (`PUT /projects/{id}/env`) is completely broken. The API writes to `deployhub/myproject/` but the ExternalSecret reads from `deployhub/myproject/{deployment-uuid}`. The `deployment_name` is a UUID, so the secret will always be empty.

---

#### 15. ECR pull token is a static placeholder, never rotated

**File:** [terraform/main.tf:248-251](file:///c:/Project/DeployHub/terraform/main.tf#L248-L251)
...
**File:** [terraform/main.tf:177](file:///c:/Project/DeployHub/terraform/main.tf#L177)
...
**File:** [terraform/security.tf:29](file:///c:/Project/DeployHub/terraform/security.tf#L29)
...
**File:** [api/main.py:98-104](file:///c:/Project/DeployHub/api/main.py#L98-L104)
...
#### 19. `deployhub_deployment_success_total` is only in the Controller

The controller increments `deployhub_deployment_success_total` when it sees replicas become ready. But if the controller restarts, the counter resets to 0. This metric will be inaccurate across pod restarts. Consider persisting it in PostgreSQL or using a Prometheus `increase()` instead of relying on the counter's absolute value.

---

#### 20. `_user_id_or_ip` rate limiter function is dead code

**File:** [api/main.py:72-84](file:///c:/Project/DeployHub/api/main.py#L72-L84)
...
**File:** [api/schema.sql:23-34](file:///c:/Project/DeployHub/api/schema.sql#L23-L34)
